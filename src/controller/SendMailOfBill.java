...
/**
 * Servlet implementation class SendMailOfBill
 */
@WebServlet("/SendMailOfBill")
public class SendMailOfBill extends HttpServlet {
...
HttpSession session1=request.getSession();
    	...
    	ResultSet rs=null;
    	String content="",sender="",recipient="",subject="";
        try {
        	HttpSession session12=request.getSession();
        	DAO dao = new DAO();
        	String aa = (String)session12.getAttribute("billnobill");
        	rs = (ResultSet)dao.getSalesReportByBillNo(aa);
        
			rs.next();
			 sender = "prajapatidhruv736@gmail.com"; //replace this with a valid sender email address
		     recipient =rs.getString("c_email"); //replace this with a valid recipient email address
		     content = "Mail by XYZ PVT LTD"; //this will be the text of the email
		     subject = "Bill Invoice"; //this will be the subject of the email
        ...
        try {
          ...
          //  preface.add(new Paragraph("Report generated by: " + System.getProperty("user.name") + ", " + new Date(), smallBold));
            ...
            preface = new Paragraph("Bill To :   "+rs.getString("c_name"),f);
            preface.add(new Chunk(glue));
            preface.add(new Paragraph("Invoice No :                   "+(rs.getString("bill_id"))));
            document.add(preface);
            
            DateFormat df1=new SimpleDateFormat("dd/MM/yyyy");
            Date date=new Date();
            preface.setSpacingBefore(50);
            preface = new Paragraph("                 "+rs.getString("c_phone"),f);
            preface.add(new Chunk(glue));
    		...
    		try {
    			while(rs.next()) 
    			{
    	            ...
    	            qty = qty+rs.getInt("qty");
    	            c1 = new PdfPCell(new Phrase(String.valueOf(rs.getInt("qty")),f));
    	            table.addCell(c1);
    	            
    	            disc=disc+rs.getDouble("disc");
    	        	c1 = new PdfPCell(new Phrase(String.valueOf(rs.getDouble("disc")),f));
    	            table.addCell(c1);
    	            ...
    	            total = total+rs.getDouble("grand_total");
    	        	c1 = new PdfPCell(new Phrase(String.valueOf(rs.getDouble("grand_total")),f));
    	            table.addCell(c1);
    			}
